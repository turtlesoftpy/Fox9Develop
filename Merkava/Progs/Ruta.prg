   ...
   FUNCTION SetNombre
      ...
      * inicio { validación de parámetro }
      ...
      IF LEN(tcNombre) > 50 THEN
         MESSAGEBOX([La longitud del parámetro 'tcNombre' debe ser como máximo de 50 caracteres.], 0+16, THIS.Name + '.SetNombre()')
         RETURN .F.
      ENDIF
   ...
   FUNCTION BuscarPorCodigo
      ...
      * fin { validación de parámetro }
      ...
      CASE _TALLY = 1   && Se ha encontrado un registro.
         WITH THIS
            .nCodigo = EVALUATE(THIS.cClavePrimaria)
            .cNombre = nombre
            .lVigente = vigente
         ENDWITH
   ...
   * ---------------------------------------------------------------------------- *
   FUNCTION BuscarPorNombre
      ...
      * inicio { validación de parámetro }
      IF VARTYPE(tcNombre) = 'C' THEN
         IF LEN(tcNombre) > 50 THEN
            MESSAGEBOX([La longitud del parámetro 'tcNombre' debe ser como máximo de 50 caracteres.], 0+16, THIS.Name + '.BuscarPorNombre()')
            RETURN .F.
         ENDIF
   ...
   * ---------------------------------------------------------------------------- *
   FUNCTION Agregar
      ...
      IF THIS.Validar() THEN
         INSERT INTO (THIS.cTabla) (id_ruta, nombre, vigente) ;
