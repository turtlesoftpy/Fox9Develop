   ...
   * ---------------------------------------------------------------------------- *
   PROTECTED FUNCTION Init
      WITH THIS
         ...
         .cClaveForanea = 'deposito'
         .cClaveForanea2 = 'almacen'
  ...
  * ---------------------------------------------------------------------------- *
   ...
   * ---------------------------------------------------------------------------- *
   FUNCTION SetVenta
      ...
      * inicio { validación de parámetro }
      IF VARTYPE(tlVenta) <> 'L' THEN
         MESSAGEBOX([El parámetro 'tlVenta' debe ser de tipo lógico.], 0+16, THIS.Name + '.SetVenta()')
         RETURN .F.
      ENDIF
      * fin { validación de parámetro }
   ...
   * ---------------------------------------------------------------------------- *
   FUNCTION ValidarCodigo()
      IF !BETWEEN(THIS.nCodigo, 1, 999) THEN
         MESSAGEBOX([El código debe ser un valor entre 1 y 999.], 0+16, THIS.Name + '.ValidarCodigo()')
         RETURN .F.
      ENDIF
   ...
   * ---------------------------------------------------------------------------- *
   FUNCTION ValidarNombre()
      ...
      llRetorno = .T.
      loModelo = NEWOBJECT(THIS.Name, THIS.Name + '.prg')
      llExiste = loModelo.BuscarPorNombre(THIS.cNombre, 'sucursal = ' + ALLTRIM(STR(THIS.nSucursal)))
   ...
   * ---------------------------------------------------------------------------- *
   FUNCTION ValidarSucursal()
      IF !BETWEEN(THIS.nSucursal, 1, 999) THEN
         MESSAGEBOX([La sucursal debe ser un valor entre 1 y 999.], 0+16, THIS.Name + '.ValidarSucursal()')
         RETURN .F.
      ENDIF
      ...
      CASE THIS.nBandera = 1   && Agregar.
         IF llExiste THEN
            MESSAGEBOX('El depósito ya existe.', 0+16, THIS.Name + '.ValidarSucursal()')
            llRetorno = .F.
         ENDIF
      CASE THIS.nBandera = 2   && Modificar.
         IF llExiste AND loModelo.GetCodigo() <> THIS.nCodigo THEN
   ...
   * ---------------------------------------------------------------------------- *
   FUNCTION Agregar
      ...
      IF THIS.Validar() THEN
         INSERT INTO (THIS.cTabla) (codigo, nombre, sucursal, venta, vigente) ;
            VALUES (THIS.nCodigo, THIS.cNombre, THIS.nSucursal, THIS.lVenta, THIS.lVigente)
   ...
   * ---------------------------------------------------------------------------- *
   FUNCTION Borrar
      ...
      * inicio { validación de parámetro }
      IF VARTYPE(tnCodigo) = 'N' THEN
         IF !BETWEEN(tnCodigo, 1, 999) THEN
            MESSAGEBOX([El parámetro 'tnCodigo' debe ser un valor entre 1 y 999.], 0+16, THIS.Name + '.Borrar()')
            RETURN .F.
         ENDIF
      ENDIF
      * fin { validación de parámetro }
      ...
      * Máquinas Terminadas.
      IF llRetorno THEN
         lcSQL = 'SELECT TOP 1 ' + THIS.cClaveForanea2 + ' FROM cabemot ORDER BY ' + THIS.cClaveForanea2 + ' WHERE ' + THIS.cClaveForanea2 + ' = ' + ALLTRIM(STR(lnCodigo)) + ' INTO CURSOR ' + lcCursor
         &lcSQL
         IF _TALLY > 0 THEN
            WAIT CLEAR
      ...
      * Presupuestos de Órdenes de Trabajo.
      IF llRetorno THEN
         lcSQL = 'SELECT TOP 1 ' + THIS.cClaveForanea2 + ' FROM cabemot2 ORDER BY ' + THIS.cClaveForanea2 + ' WHERE ' + THIS.cClaveForanea2 + ' = ' + ALLTRIM(STR(lnCodigo)) + ' INTO CURSOR ' + lcCursor
         &lcSQL
         IF _TALLY > 0 THEN
            WAIT CLEAR
