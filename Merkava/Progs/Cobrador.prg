DEFINE CLASS Cobrador AS CUSTOM
   * Propiedades.
   PROTECTED nBandera
...
*!*	            .cDocumento = documento
   ...
   * ---------------------------------------------------------------------------- *
   FUNCTION Borrar
      ...
      * Cobros.
      IF llRetorno THEN
         lcSQL = 'SELECT TOP 1 ' + THIS.cClaveForanea + ' FROM cabecob ORDER BY ' + THIS.cClaveForanea + ' WHERE ' + THIS.cClaveForanea + ' = ' + ALLTRIM(STR(lnCodigo)) + ' INTO CURSOR ' + lcCursor
         &lcSQL
         IF _TALLY > 0 THEN
            WAIT CLEAR
            MESSAGEBOX([El registro figura en el archivo de 'Cobros a clientes', imposible borrarlo.], 0+16, THIS.Name + '.Borrar()')
            llRetorno = .F.
         ENDIF
      ENDIF
