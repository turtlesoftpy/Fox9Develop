DEFINE CLASS UnidadMedida AS CUSTOM
   * Propiedades.
   PROTECTED nBandera
   ...
   * ---------------------------------------------------------------------------- *
   FUNCTION SetSimbolo
      LPARAMETER tcSimbolo

      * inicio { validación de parámetro }
      IF VARTYPE(tcSimbolo) <> 'C' THEN
         MESSAGEBOX([El parámetro 'tcSimbolo' debe ser de tipo texto.], 0+16, THIS.Name + '.SetSimbolo()')
         RETURN .F.
      ENDIF

      IF LEN(tcSimbolo) > 5 THEN
         MESSAGEBOX([La longitud del parámetro 'tcSimbolo' debe ser como máximo de 30 caracteres.], 0+16, THIS.Name + '.SetSimbolo()')
         RETURN .F.
      ENDIF

      IF AT('[', tcSimbolo) > 0 THEN
         MESSAGEBOX('Los corchetes no están permitidos.', 0+16, THIS.Name + '.SetSimbolo()')
         RETURN .F.
      ENDIF
   ...
   * ---------------------------------------------------------------------------- *
   FUNCTION SetDivisible
      LPARAMETER tlDivisible

      * inicio { validación de parámetro }
      IF VARTYPE(tlDivisible) <> 'L' THEN
         MESSAGEBOX([El parámetro 'tlDivisible' debe ser de tipo lógico.], 0+16, THIS.Name + '.SetDivisible()')
         RETURN .F.
      ENDIF
      * fin { validación de parámetro }
   ...
   * ---------------------------------------------------------------------------- *
   FUNCTION BuscarPorSimbolo
      LPARAMETER tcSimbolo, tcCondicionFiltrado, tcCursor

      * inicio { validación de parámetro }
      IF VARTYPE(tcSimbolo) = 'C' THEN
         IF LEN(tcSimbolo) > 5 THEN
            MESSAGEBOX([La longitud del parámetro 'tcSimbolo' debe ser como máximo de 5 caracteres.], 0+16, THIS.Name + '.BuscarPorSimbolo()')
            RETURN .F.
         ENDIF

         IF AT('[', tcSimbolo) > 0 THEN
            MESSAGEBOX('Los corchetes no están permitidos.', 0+16, THIS.Name + '.BuscarPorSimbolo()')
            RETURN .F.
         ENDIF
      ...
      IF EMPTY(IIF(!INLIST(VARTYPE(tcSimbolo), 'L', 'X'), tcSimbolo, THIS.cSimbolo)) THEN
         RETURN .F.
      ENDIF
      ...
      * fin { validación de parámetro }
      ...
      LOCAL llRetorno, lcCursor, lcSimbolo, lcSQL
      llRetorno = .T.
      lcCursor = CreaTemp()
      lcSimbolo = IIF(!INLIST(VARTYPE(tcSimbolo), 'L', 'X'), ALLTRIM(tcSimbolo), ALLTRIM(THIS.cSimbolo))

      IF ISNULL(tcCondicionFiltrado) THEN
         lcSQL = 'SELECT * FROM ' + THIS.cTabla + ' WHERE UPPER(simbolo) LIKE [' + ALLTRIM(UPPER(lcSimbolo)) + '] INTO CURSOR ' + lcCursor
      ELSE
         lcSQL = 'SELECT * FROM ' + THIS.cTabla + ' WHERE UPPER(simbolo) LIKE [' + ALLTRIM(UPPER(lcSimbolo)) + '] AND ' + tcCondicionFiltrado + ' INTO CURSOR ' + lcCursor
      ENDIF
      ...
      CASE _TALLY = 1   && Se ha encontrado un registro.
         ...
         IF VARTYPE(tcCursor) = 'C' THEN
            IF EMPTY(tcCursor) THEN
               MESSAGEBOX([El parámetro 'tcCursor' no debe estar vacío.], 0+16, THIS.Name + '.BuscarPorSimbolo()')
               llRetorno = .F.
            ELSE
               * Copia todos los registros al cursor que se ha pasado como parámetro.
               SELECT * FROM (lcCursor) INTO CURSOR (tcCursor)
      ...
      CASE _TALLY > 1   && Se ha encontrado más de un registro.
         IF VARTYPE(tcCursor) <> 'C' THEN
            MESSAGEBOX([El parámetro 'tcCursor' debe ser de tipo texto.], 0+16, THIS.Name + '.BuscarPorSimbolo()')
            llRetorno = .F.
         ELSE
            IF EMPTY(tcCursor) THEN
               MESSAGEBOX([El parámetro 'tcCursor' no debe estar vacío.], 0+16, THIS.Name + '.BuscarPorSimbolo()')
   ...
   * ---------------------------------------------------------------------------- *
   FUNCTION ValidarSimbolo()
      IF EMPTY(THIS.cSimbolo) THEN
         MESSAGEBOX('El simbolo no puede quedar en blanco.', 0+16, THIS.Name + '.ValidarSimbolo()')
         RETURN .F.
      ENDIF
      ...
      llRetorno = .T.
      loModelo = NEWOBJECT(THIS.Name, THIS.Name + '.prg')
      llExiste = loModelo.BuscarPorSimbolo(THIS.cSimbolo)

      DO CASE
      CASE THIS.nBandera = 1   && Agregar.
         IF llExiste THEN
            MESSAGEBOX('El símbolo ya existe.', 0+16, THIS.Name + '.ValidarSimbolo()')
            llRetorno = .F.
         ENDIF
      CASE THIS.nBandera = 2   && Modificar.
         IF llExiste AND loModelo.GetCodigo() <> THIS.nCodigo THEN
   ...
   * ---------------------------------------------------------------------------- *
   FUNCTION Agregar
      ...
      IF THIS.Validar() THEN
         INSERT INTO (THIS.cTabla) (codigo, nombre, simbolo, divisible, vigente) ;
            VALUES (THIS.nCodigo, THIS.cNombre, THIS.cSimbolo, THIS.lDivisible, THIS.lVigente)
   ...
   * ---------------------------------------------------------------------------- *
   FUNCTION Modificar
      ...
      IF THIS.Validar() THEN
         ...
         SET ;
            nombre = THIS.cNombre, ;
            simbolo = THIS.cSimbolo, ;
            divisible = THIS.lDivisible, ;
