  ...
  * ---------------------------------------------------------------------------- *
   ...
   * ---------------------------------------------------------------------------- *
   FUNCTION ValidarNombre()
      ...
      llRetorno = .T.
      loModelo = NEWOBJECT(THIS.Name, THIS.Name + '.prg')
      llExiste = loModelo.BuscarPorNombre(THIS.cNombre, 'departamen = ' + ALLTRIM(STR(THIS.nDepartamen)))
   ...
   * ---------------------------------------------------------------------------- *
   FUNCTION ValidarDepartamen()
      ...
      CASE THIS.nBandera = 1   && Agregar.
         IF llExiste THEN
            MESSAGEBOX('La ciudad ya existe.', 0+16, THIS.Name + '.ValidarDepartamen()')
            llRetorno = .F.
         ENDIF
      CASE THIS.nBandera = 2   && Modificar.
         IF llExiste AND loModelo.GetCodigo() <> THIS.nCodigo THEN
   ...
   * ---------------------------------------------------------------------------- *
   FUNCTION Agregar
      ...
      IF THIS.Validar() THEN
         INSERT INTO (THIS.cTabla) (codigo, nombre, departamen, vigente) ;
            VALUES (THIS.nCodigo, THIS.cNombre, THIS.nDepartamen, THIS.lVigente)
   ...
   * ---------------------------------------------------------------------------- *
   FUNCTION Borrar
      ...
      * inicio { validación de parámetro }
      IF VARTYPE(tnCodigo) = 'N' THEN
         IF !BETWEEN(tnCodigo, 1, 99999) THEN
            MESSAGEBOX([El parámetro 'tnCodigo' debe ser un valor entre 1 y 99999.], 0+16, THIS.Name + '.Borrar()')
            RETURN .F.
         ENDIF
      ENDIF
      * fin { validación de parámetro }
