   ...
   * ---------------------------------------------------------------------------- *
   FUNCTION BuscarPorSimbolo
      ...
      * Guarda el área de trabajo original.
      ...
      IF ISNULL(tcCondicionFiltrado) THEN
         goCapaDatos.LlamarConsulta('SELECT * FROM ' + THIS.cTabla + ' WHERE UPPER(simbolo) LIKE ?pcSimbolo', lcCursor)
      ELSE
         goCapaDatos.LlamarConsulta('SELECT * FROM ' + THIS.cTabla + ' WHERE UPPER(simbolo) LIKE ?pcSimbolo AND ' + tcCondicionFiltrado, lcCursor)
      ENDIF
   ...
   * ---------------------------------------------------------------------------- *
   FUNCTION ValidarSimbolo()
      IF EMPTY(THIS.cSimbolo) THEN
         MESSAGEBOX('El símbolo no puede quedar en blanco.', 0+16, THIS.Name + '.ValidarSimbolo()')
         RETURN .F.
      ENDIF

      IF LEN(THIS.cSimbolo) > 5 THEN
         MESSAGEBOX([La longitud del parámetro 'tcSimbolo' debe ser como máximo de 5 caracteres.], 0+16, THIS.Name + '.ValidarSimbolo()')
         RETURN .F.
      ENDIF
   ...
   FUNCTION Agregar
      ...
      * Guarda el área de trabajo original.
      ...
      IF THIS.Validar() THEN
         PRIVATE pnCodigo, pcNombre, pcSimbolo, plDivisible, plVigente
         ...
         IF goCapaDatos.AgregarRegistro(THIS.cTabla, ;
                                        ...
                                        '?pnCodigo, ?pcNombre, ?pcSimbolo, ?plDivisible, ?plVigente')
