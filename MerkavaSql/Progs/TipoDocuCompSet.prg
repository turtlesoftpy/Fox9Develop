DEFINE CLASS TipoDocuCompSet AS CUSTOM
   * Propiedades.
   PROTECTED nBandera
   ...
   * ---------------------------------------------------------------------------- *
   PROTECTED FUNCTION Init
      WITH THIS
         .nBandera = 0
         .cTabla = 'tipodocucomp_set'
         .cClavePrimaria = 'codigo'
         .cClaveForanea = 'tipodocucomp_set'
   ...
   * ---------------------------------------------------------------------------- *
   FUNCTION Borrar
      ...
      * Tipos de Documentos de Compras de la SET.
      IF llRetorno THEN
         lcTablaRelacionada = 'timbradocomp'
         ...
         IF lnCantidadRegistro > 0 THEN
            WAIT CLEAR
            MESSAGEBOX([El registro figura en el archivo de 'Tipos de documentos de compras de la SET', imposible borrarlo.], 0+16, THIS.Name + '.Borrar()')
            llRetorno = .F.
         ENDIF
      ENDIF
